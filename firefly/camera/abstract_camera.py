...
def _generic_camera(width: int, height: int, sampler: Sampler, generator: CameraRayGenerator, nb_sample=10) -> Camera:
    def __generate_image(integrator: Integrator):
        ...
        samples = [integrator(generator(sampler(1 / width, 1 / height, uv_screen_pos)), 0) for _ in range(nb_sample)]
        result = np.sum(samples, axis=0) * 1 / nb_sample
