...
def Integrator(scene: Scene, light: Light) -> Integrator:
    def compute_material(surface_data: SurfaceData, mask: np.ndarray, wi: np.ndarray, integrator: Integrator, __step=0):
        ...
        for material_id, (rho, _) in enumerate(surface_data.material_list):
            ...
            cumulated_light[material_mask] += \
                rho(position[material_mask], normal[material_mask], wi[mask][material_mask], integrator, __step) \
    ...
    def __integrate(ray: Ray, __step=0):
        ...
        cumulated_light[mask] = compute_material(surface_data, mask, ray[1], __integrate, __step)
